<template>
  <div class="bg-gray-50" id="pricing">
    <div
      class="relative bg-cover bg-center "
      style="background-image:url('https://images.unsplash.com/photo-1620121478247-ec786b9be2fa?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=3432&q=80')"
    >
      <div
        aria-hidden="true"
        class="absolute bottom-0 hidden h-6 w-full bg-gray-50 lg:block"
      ></div>

      <div
        class="
          relative
          mx-auto
          max-w-2xl
          px-4
          pt-16
          text-center
          sm:px-6 sm:pt-32
          lg:max-w-7xl lg:px-8
        "
      >
        <h1 class="text-4xl font-bold tracking-tight text-white sm:text-6xl">
          <span class="block lg:inline">Simple workflow-based pricing. </span>
        </h1>
        <p class="mt-4 text-xl text-indigo-100">
          Everything you need, nothing you don't. Pick a plan that best suits
          your business.
        </p>
      </div>

      <h2 class="sr-only">Plans</h2>

      <!-- Toggle -->
      <div class="relative mt-12 flex justify-center sm:mt-16">
        <div class="flex rounded-lg bg-black/60 backdrop-blur p-0.5">
          <button
            type="button"
            @click="billingCycle = !billingCycle"
            :class="[
              !billingCycle
                ? 'bg-white text-indigo-700 hover:bg-indigo-50 border border-indigo-700'
                : 'text-indigo-200 hover:bg-indigo-800 border-transparent',
              `relative
              whitespace-nowrap
              rounded-md
              
              py-2
              px-6
              text-sm
              font-medium
              shadow-sm
        
              focus:outline-none
              
              `,
            ]"
          >
            Quarterly billing
          </button>
          <button
            @click="billingCycle = !billingCycle"
            type="button"
            :class="[
              billingCycle
                ? 'bg-white text-indigo-700 hover:bg-indigo-50 border border-indigo-700'
                : 'text-indigo-200 hover:bg-indigo-800 border-transparent',
              `relative
              ml-0.5
              whitespace-nowrap
              rounded-md
              py-2
              px-6
              text-sm
              font-medium
              `,
            ]"
          >
            Yearly billing
          </button>
        </div>
      </div>

      <!-- Cards -->
      <div
        class="
          relative
          mx-auto
          mt-8
          max-w-2xl
          px-4
          pb-8
          sm:mt-12 sm:px-6
          lg:max-w-7xl lg:px-8 lg:pb-0
        "
      >
        <!-- Decorative background -->
        <div
          aria-hidden="true"
          class="
            absolute
            inset-0
            top-4
            bottom-6
            left-8
            right-8
            hidden
            rounded-tl-lg rounded-tr-lg
            bg-black/60
            backdrop-blur
            lg:block
          "
        />

        <div class="relative space-y-6 lg:grid lg:grid-cols-3 lg:space-y-0">
          <div
            v-for="plan in plans"
            :key="plan.title"
            :class="[
              plan.featured
                ? 'bg-white ring-2 ring-cyan-500 shadow-md'
                : 'bg-indigo-700 lg:bg-transparent',
              'pt-6 px-6 pb-3 rounded-lg lg:px-8 lg:pt-12',
            ]"
          >
            <div>
              <h3
                :class="[
                  plan.featured ? 'text-cyan-500' : 'text-white',
                  'text-base font-semibold',
                ]"
              >
                {{ plan.title }}
              </h3>
              <div
                class="
                  flex flex-col
                  items-start
                  sm:flex-row sm:items-center sm:justify-between
                  lg:flex-col lg:items-start
                "
              >
                <div class="mt-3 flex items-center">
                  <p
                    :class="[
                      plan.featured ? 'text-cyan-500' : 'text-white',
                      'text-4xl font-bold tracking-tight',
                    ]"
                  >
                    ${{
                      (
                        plan.priceMonthly * (billingCycle ? 1 : 1.2)
                      ).toLocaleString()
                    }}
                  </p>
                  <div class="ml-4">
                    <p
                      :class="[
                        plan.featured ? 'text-gray-700' : 'text-white',
                        'text-sm',
                      ]"
                    >
                      USD / mo
                    </p>
                    <p
                      :class="[
                        plan.featured ? 'text-gray-500' : 'text-indigo-200',
                        'text-sm',
                      ]"
                    >
                      Billed {{ billingCycle ? "yearly" : "quarterly" }} (${{
                        billingCycle
                          ? plan.priceYearly.toLocaleString()
                          : (plan.priceMonthly * 1.2 * 3).toLocaleString()
                      }})
                    </p>
                  </div>
                </div>
                <a
                  @click="setPlan(plan.title)"
                  href="/#pricing"
                  :class="[
                    plan.featured
                      ? 'bg-cyan-500 text-white hover:bg-cyan-700'
                      : 'bg-white text-cyan-600 hover:bg-indigo-50',
                    'mt-6 w-full inline-block py-2 px-8 border border-transparent rounded-md shadow-sm text-center text-sm font-medium sm:mt-0 sm:w-auto lg:mt-6 lg:w-full',
                  ]"
                >
                  {{ plan.featured ? "Suggested plan" : `Buy ${plan.title}` }}
                </a>
              </div>
            </div>
            <h4 class="sr-only">Features</h4>
            <ul
              role="list"
              :class="[
                plan.featured
                  ? 'border-gray-200 divide-gray-200'
                  : 'border-sky-500 divide-sky-500 divide-opacity-75',
                'mt-7 border-t divide-y lg:border-t-0',
              ]"
            >
              <li
                v-for="mainFeature in plan.mainFeatures"
                :key="mainFeature.id"
                class="flex items-center py-3"
              >
                <CheckIcon
                  :class="[
                    plan.featured ? 'text-indigo-500' : 'text-indigo-200',
                    'w-5 h-5 flex-shrink-0',
                  ]"
                  aria-hidden="true"
                />
                <span
                  :class="[
                    plan.featured ? 'text-gray-600' : 'text-white',
                    'ml-3 text-sm font-medium',
                  ]"
                  >{{ mainFeature.value }}</span
                >
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>

    <!-- Feature comparison (up to lg) -->
    <section aria-labelledby="mobile-comparison-heading" class="lg:hidden">
      <h2 id="mobile-comparison-heading" class="sr-only">Feature comparison</h2>

      <div class="mx-auto max-w-2xl space-y-16 py-16 px-4 sm:px-6">
        <div
          v-for="(plan, mobilePlanIndex) in plans"
          :key="mobilePlanIndex"
          class="border-t border-gray-200"
        >
          <div
            :class="[
              plan.featured ? 'border-cyan-600' : 'border-transparent',
              '-mt-px pt-6 border-t-2 sm:w-1/2',
            ]"
          >
            <h3
              :class="[
                plan.featured ? 'text-cyan-600' : 'text-gray-900',
                'text-sm font-bold',
              ]"
            >
              {{ plan.title }}
            </h3>
            <p class="mt-2 text-sm text-gray-500">{{ plan.description }}</p>
          </div>
          <h4 class="mt-10 text-sm font-bold text-gray-900">
            Workspace Limits
          </h4>

          <div class="relative mt-6">
            <!-- Fake card background -->
            <div
              aria-hidden="true"
              class="pointer-events-none absolute inset-0 hidden sm:block"
            >
              <div
                :class="[
                  plan.featured ? 'shadow-md' : 'shadow',
                  'absolute right-0 w-1/2 h-full bg-white rounded-lg',
                ]"
              />
            </div>

            <div
              :class="[
                plan.featured
                  ? 'ring-2 ring-cyan-600 shadow-md'
                  : 'ring-1 ring-black ring-opacity-5 shadow',
                'relative py-3 px-4 bg-white rounded-lg sm:p-0 sm:bg-transparent sm:rounded-none sm:ring-0 sm:shadow-none',
              ]"
            >
              <dl class="divide-y divide-gray-200">
                <div
                  v-for="feature in features"
                  :key="feature.title"
                  class="
                    flex
                    items-center
                    justify-between
                    py-3
                    sm:grid sm:grid-cols-2
                  "
                >
                  <dt class="pr-4 text-sm font-medium text-gray-600">
                    {{ feature.title }}
                  </dt>
                  <dd
                    class="
                      flex
                      items-center
                      justify-end
                      sm:justify-center sm:px-4
                    "
                  >
                    <span
                      v-if="
                        typeof feature.tiers[mobilePlanIndex].value === 'string'
                      "
                      :class="[
                        feature.tiers[mobilePlanIndex].featured
                          ? 'text-cyan-600'
                          : 'text-gray-900',
                        'text-sm font-medium',
                      ]"
                      >{{ feature.tiers[mobilePlanIndex].value }}</span
                    >
                    <template v-else>
                      <CheckIcon
                        v-if="feature.tiers[mobilePlanIndex].value === true"
                        class="mx-auto h-5 w-5 text-cyan-600"
                        aria-hidden="true"
                      />
                      <XMarkIcon
                        v-else
                        class="mx-auto h-5 w-5 text-gray-400"
                        aria-hidden="true"
                      />
                      <span class="sr-only">{{
                        feature.tiers[mobilePlanIndex].value === true
                          ? "Yes"
                          : "No"
                      }}</span>
                    </template>
                  </dd>
                </div>
              </dl>
            </div>

            <!-- Fake card border -->
            <div
              aria-hidden="true"
              class="pointer-events-none absolute inset-0 hidden sm:block"
            >
              <div
                :class="[
                  plan.featured
                    ? 'ring-2 ring-cyan-600'
                    : 'ring-1 ring-black ring-opacity-5',
                  'absolute right-0 w-1/2 h-full rounded-lg',
                ]"
              />
            </div>
          </div>

          <h4 class="mt-10 text-sm font-bold text-gray-900">Features</h4>

          <div class="relative mt-6">
            <!-- Fake card background -->
            <div
              aria-hidden="true"
              class="pointer-events-none absolute inset-0 hidden sm:block"
            >
              <div
                :class="[
                  plan.featured ? 'shadow-md' : 'shadow',
                  'absolute right-0 w-1/2 h-full bg-white rounded-lg',
                ]"
              />
            </div>

            <div
              :class="[
                plan.featured
                  ? 'ring-2 ring-cyan-600 shadow-md'
                  : 'ring-1 ring-black ring-opacity-5 shadow',
                'relative py-3 px-4 bg-white rounded-lg sm:p-0 sm:bg-transparent sm:rounded-none sm:ring-0 sm:shadow-none',
              ]"
            >
              <dl class="divide-y divide-gray-200">
                <div
                  v-for="perk in perks"
                  :key="perk.title"
                  class="flex justify-between py-3 sm:grid sm:grid-cols-2"
                >
                  <dt class="text-sm font-medium text-gray-600 sm:pr-4">
                    {{ perk.title }}
                  </dt>
                  <dd class="text-center sm:px-4">
                    <CheckIcon
                      v-if="perk.tiers[mobilePlanIndex].value === true"
                      class="mx-auto h-5 w-5 text-indigo-600"
                      aria-hidden="true"
                    />
                    <XMarkIcon
                      v-else
                      class="mx-auto h-5 w-5 text-gray-400"
                      aria-hidden="true"
                    />
                    <span class="sr-only">{{
                      perk.tiers[mobilePlanIndex].value === true ? "Yes" : "No"
                    }}</span>
                  </dd>
                </div>
              </dl>
            </div>

            <!-- Fake card border -->
            <div
              aria-hidden="true"
              class="pointer-events-none absolute inset-0 hidden sm:block"
            >
              <div
                :class="[
                  plan.featured
                    ? 'ring-2 ring-indigo-600'
                    : 'ring-1 ring-black ring-opacity-5',
                  'absolute right-0 w-1/2 h-full rounded-lg',
                ]"
              />
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- Feature comparison (lg+) -->
    <section aria-labelledby="comparison-heading" class="hidden lg:block">
      <h2 id="comparison-heading" class="sr-only">Feature comparison</h2>

      <div class="mx-auto max-w-7xl py-24 px-8">
        <div class="flex w-full items-stretch border-t border-gray-200">
          <div class="-mt-px flex w-1/4 items-end py-6 pr-4">
            <h3 class="mt-auto text-sm font-bold text-gray-900">
              Workspace Limits
            </h3>
          </div>
          <div
            v-for="(plan, planIdx) in plans"
            :key="plan.title"
            aria-hidden="true"
            :class="[
              planIdx === plans.length - 1 ? '' : 'pr-4',
              '-mt-px pl-4 w-1/4',
            ]"
          >
            <div
              :class="[
                plan.featured ? 'border-indigo-600' : 'border-transparent',
                'py-6 border-t-2',
              ]"
            >
              <p
                :class="[
                  plan.featured ? 'text-indigo-600' : 'text-gray-900',
                  'text-sm font-bold',
                ]"
              >
                {{ plan.title }}
              </p>
              <p class="mt-2 text-sm text-gray-500">{{ plan.description }}</p>
            </div>
          </div>
        </div>

        <div class="relative">
          <!-- Fake card backgrounds -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 pr-4" />
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow-md" />
            </div>
            <div class="w-1/4 pl-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
          </div>

          <table class="relative w-full">
            <caption class="sr-only">
              Business feature comparison
            </caption>
            <thead>
              <tr class="text-left">
                <th scope="col">
                  <span class="sr-only">Feature</span>
                </th>
                <th v-for="plan in plans" :key="plan.title" scope="col">
                  <span class="sr-only">{{ plan.title }} plan</span>
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="feature in features" :key="feature.title">
                <th
                  scope="row"
                  class="
                    w-1/4
                    py-3
                    pr-4
                    text-left text-sm
                    font-medium
                    text-gray-600
                  "
                >
                  {{ feature.title }}
                </th>
                <td
                  v-for="(tier, tierIdx) in feature.tiers"
                  :key="tier.title"
                  :class="[
                    tierIdx === feature.tiers.length - 1 ? 'pl-4' : 'px-4',
                    'relative w-1/4 py-0 text-center',
                  ]"
                >
                  <span class="relative h-full w-full py-3">
                    <span
                      v-if="typeof tier.value === 'string'"
                      :class="[
                        tier.featured ? 'text-indigo-600' : 'text-gray-900',
                        'text-sm font-medium',
                      ]"
                      >{{ tier.value }}</span
                    >
                    <template v-else>
                      <CheckIcon
                        v-if="tier.value === true"
                        class="mx-auto h-5 w-5 text-indigo-600"
                        aria-hidden="true"
                      />
                      <XMarkIcon
                        v-else
                        class="mx-auto h-5 w-5 text-gray-400"
                        aria-hidden="true"
                      />
                      <span class="sr-only">{{
                        tier.value === true ? "Yes" : "No"
                      }}</span>
                    </template>
                  </span>
                </td>
              </tr>
            </tbody>
          </table>

          <!-- Fake card borders -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 pr-4" />
            <div
              v-for="(plan, idx) in plans"
              :class="[idx < 2 ? 'px-4' : `pl-4`, 'w-1/4']"
              :key="plan.description"
            >
              <div
                :class="[
                  plan.featured
                    ? 'ring-2 ring-indigo-600 ring-opacity-100'
                    : 'ring-1 ring-black ring-opacity-5',
                  `h-full
                  w-full
                  rounded-lg`,
                ]"
              />
            </div>
          </div>
        </div>

        <h3 class="mt-10 text-sm font-bold text-gray-900">Other perks</h3>

        <div class="relative mt-6">
          <!-- Fake card backgrounds -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 pr-4" />
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow-md" />
            </div>
            <div class="w-1/4 pl-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
          </div>

          <table class="relative w-full">
            <caption class="sr-only">
              Perk comparison
            </caption>
            <thead>
              <tr class="text-left">
                <th scope="col">
                  <span class="sr-only">Perk</span>
                </th>
                <th v-for="plan in plans" :key="plan.title" scope="col">
                  <span class="sr-only">{{ plan.title }} plan</span>
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="perk in perks" :key="perk.title">
                <th
                  scope="row"
                  class="
                    w-1/4
                    py-3
                    pr-4
                    text-left text-sm
                    font-medium
                    text-gray-600
                  "
                >
                  {{ perk.title }}
                </th>
                <td
                  v-for="(tier, tierIdx) in perk.tiers"
                  :key="tier.title"
                  :class="[
                    tierIdx === perk.tiers.length - 1 ? 'pl-4' : 'px-4',
                    'relative w-1/4 py-0 text-center',
                  ]"
                >
                  <span
                    v-if="typeof tier.value === 'string'"
                    :class="[
                      tier.featured ? 'text-indigo-600' : 'text-gray-900',
                      'text-sm font-medium',
                    ]"
                    >{{ tier.value }}</span
                  >
                  <template v-else
                    ><span class="relative h-full w-full py-3">
                      <CheckIcon
                        v-if="tier.value === true"
                        class="mx-auto h-5 w-5 text-indigo-600"
                        aria-hidden="true"
                      />
                      <XMarkIcon
                        v-else
                        class="mx-auto h-5 w-5 text-gray-400"
                        aria-hidden="true"
                      />
                      <span class="sr-only">{{
                        tier.value === true ? "Yes" : "No"
                      }}</span>
                    </span></template
                  >
                </td>
              </tr>
            </tbody>
          </table>

          <!-- Fake card borders -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 px-4" />
            <div
              v-for="(plan, idx) in plans"
              :class="[idx < 2 ? 'px-4' : `pl-4`, 'w-1/4']"
              :key="plan.description"
            >
              <div
                :class="[
                  plan.featured
                    ? 'ring-2 ring-indigo-600 ring-opacity-100'
                    : 'ring-1 ring-black ring-opacity-5',
                  `h-full
                  w-full
                  rounded-lg`,
                ]"
              />
            </div>
          </div>
        </div>
        <h3 class="mt-10 text-sm font-bold text-gray-900">Support</h3>
        <div class="relative mt-6">
          <!-- Fake card backgrounds -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 pr-4" />
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
            <div class="w-1/4 px-4">
              <div class="h-full w-full rounded-lg bg-white shadow-md" />
            </div>
            <div class="w-1/4 pl-4">
              <div class="h-full w-full rounded-lg bg-white shadow" />
            </div>
          </div>

          <table class="relative w-full">
            <caption class="sr-only">
              Perk comparison
            </caption>
            <thead>
              <tr class="text-left">
                <th scope="col">
                  <span class="sr-only">Perk</span>
                </th>
                <th v-for="plan in plans" :key="plan.title" scope="col">
                  <span class="sr-only">{{ plan.title }} plan</span>
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="support in supports" :key="support.title">
                <th
                  scope="row"
                  class="
                    w-1/4
                    py-3
                    pr-4
                    text-left text-sm
                    font-medium
                    text-gray-600
                  "
                >
                  {{ support.title }}
                </th>
                <td
                  v-for="(tier, tierIdx) in support.tiers"
                  :key="tier.title"
                  :class="[
                    tierIdx === support.tiers.length - 1 ? 'pl-4' : 'px-4',
                    'relative w-1/4 py-0 text-center',
                  ]"
                >
                  <span
                    v-if="typeof tier.value === 'string'"
                    :class="[
                      tier.featured ? 'text-indigo-600' : 'text-gray-900',
                      'text-sm font-medium',
                    ]"
                    >{{ tier.value }}</span
                  >
                  <template v-else
                    ><span class="relative h-full w-full py-3">
                      <CheckIcon
                        v-if="tier.value === true"
                        class="mx-auto h-5 w-5 text-indigo-600"
                        aria-hidden="true"
                      />
                      <XMarkIcon
                        v-else
                        class="mx-auto h-5 w-5 text-gray-400"
                        aria-hidden="true"
                      />
                      <span class="sr-only">{{
                        tier.value === true ? "Yes" : "No"
                      }}</span>
                    </span></template
                  >
                </td>
              </tr>
            </tbody>
          </table>

          <!-- Fake card borders -->
          <div
            class="pointer-events-none absolute inset-0 flex items-stretch"
            aria-hidden="true"
          >
            <div class="w-1/4 pr-4" />
            <div
              v-for="(plan, idx) in plans"
              :class="[idx < 2 ? 'px-4' : `pl-4`, 'w-1/4']"
              :key="plan.description"
            >
              <div
                :class="[
                  plan.featured
                    ? 'ring-2 ring-indigo-600 ring-opacity-100'
                    : 'ring-1 ring-black ring-opacity-5',
                  `h-full
                  w-full
                  rounded-lg`,
                ]"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script setup>
import { CheckIcon, XMarkIcon } from "@heroicons/vue/20/solid";

const features = [
  {
    title: "Users",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "Unlimited",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "Unlimited",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "Unlimited",
      },
    ],
  },
  {
    title: "Connectors",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "Unlimited",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "Unlimited",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "Unlimited",
      },
    ],
  },
  {
    title: "Workspaces",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "1",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "2",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "Unlimited",
      },
    ],
  },
  {
    title: "Workflows",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "3 included",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "5 included",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "10 included",
      },
    ],
  },
];
const perks = [
  {
    title: "SSO",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: (localStorage.getItem("tier") === "Team" && localStorage.getItem("sso") === "true") ? true : 'Add on',
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "Multi-regions",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "Add-on",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "Log streaming",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: (localStorage.getItem("tier") === "Team" && localStorage.getItem("log_streaming") === "true") ? true : 'Add on',
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "On-premise connectivity",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: (localStorage.getItem("tier") === "Team" && localStorage.getItem("on-prem") === "true") ? true : 'Add on',
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "Custom log retention",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: (localStorage.getItem("tier") === "Team" && localStorage.getItem("log_retention") === "true") ? true : 'Add on',
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "HIPAA Compliance",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: localStorage.getItem("tier") === "Professional" && localStorage.getItem("hipaa") === "true",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: (localStorage.getItem("tier") === "Team" && localStorage.getItem("hipaa") === "true") ? true : 'Add on', 
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
];

const supports = [
  {
    title: "Availability",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "17 / 5",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "17 / 5",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "24 / 5",
      },
    ],
  },
  {
    title: "SLA",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: "2 days",
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: "2 days",
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: "1 day",
      },
    ],
  },
  {
    title: "Zendesk",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: true,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: true,
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "Community Slack",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: true,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: true,
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
  {
    title: "Screen Sharing (2 hr/qtr)",
    tiers: [
      {
        title: "starter",
        featured: localStorage.getItem("tier") === "Professional",
        value: false,
      },
      {
        title: "popular",
        featured: localStorage.getItem("tier") === "Team",
        value: false,
      },
      {
        title: "intermediate",
        featured: localStorage.getItem("tier") === "Enterprise",
        value: true,
      },
    ],
  },
];
</script>

<script>
export default {
  data() {
    return {
      billingCycle: true,
    };
  },
  methods: {
    setPlan(value) {
      localStorage.setItem("tier", value);
    },
  },
  computed: {
    plans() {
      let base_plans = [
        {
          title: "Professional",
          featured: localStorage.getItem("tier") === "Professional",
          description: "All your essential business finances, taken care of.",
          priceMonthly: 1500,
          priceYearly: 18000,
          mainFeatures: [
            { id: 1, value: "1  Shared Workspace" },
            { id: 2, value: "3  Included Workflows" },
            { id: 3, value: "Unlimited users" },
          ],
        },
        {
          title: "Team",
          featured: localStorage.getItem("tier") === "Team",
          description:
            "The best financial services for your thriving business.",
          priceMonthly: 2500,
          priceYearly: 30000,
          mainFeatures: [
            { id: 1, value: "2  shared workspaces" },
            { id: 2, value: "5  Included Workflows" },
            { id: 3, value: "Unlimited users" },
          ],
        },
        {
          title: "Enterprise",
          featured: localStorage.getItem("tier") === "Enterprise",
          description:
            "Convenient features to take your business to the next level.",
          priceMonthly: 5000,
          priceYearly: 60000,
          mainFeatures: [
            { id: 1, value: "Unlimited shared workspaces" },
            { id: 2, value: "10  Included Workflows" },
            { id: 3, value: "Unlimited users" },
          ],
        },
      ];
      if (localStorage.getItem("tier") === "Team") {
        localStorage.getItem("hipaa") === "true"
          ? base_plans[1].mainFeatures.push({ id: 4, value: "HIPAA" })
          : "";
        localStorage.getItem("sso") === "true"
          ? base_plans[1].mainFeatures.push({ id: 5, value: "SSO" })
          : "";
        localStorage.getItem("on-prem") === "true"
          ? base_plans[1].mainFeatures.push({ id: 6, value: "On premise" })
          : "";
        localStorage.getItem("log_streaming") === "true"
          ? base_plans[1].mainFeatures.push({ id: 7, value: "Log streaming" })
          : "";
        localStorage.getItem("log_retention") === "true"
          ? base_plans[1].mainFeatures.push({
              id: 8,
              value: "Custom log retention",
            })
          : "";
      } else if (localStorage.getItem("tier") === "Professional") {
        localStorage.getItem("hipaa") === "true"
          ? base_plans[0].mainFeatures.push({ id: 4, value: "HIPAA" })
          : "";
        localStorage.getItem("sso") === "true"
          ? base_plans[0].mainFeatures.push({ id: 5, value: "SSO" })
          : "";
        localStorage.getItem("on-prem") === "true"
          ? base_plans[0].mainFeatures.push({ id: 6, value: "On premise" })
          : "";
        localStorage.getItem("log_streaming") === "true"
          ? base_plans[0].mainFeatures.push({ id: 7, value: "Log streaming" })
          : "";
        localStorage.getItem("log_retention") === "true"
          ? base_plans[0].mainFeatures.push({
              id: 8,
              value: "Custom log retention",
            })
          : "";
      } else if (localStorage.getItem("tier") === "Enterprise") {
        localStorage.getItem("hipaa") === "true"
          ? base_plans[2].mainFeatures.push({ id: 4, value: "HIPAA" })
          : "";
        localStorage.getItem("sso") === "true"
          ? base_plans[2].mainFeatures.push({ id: 5, value: "SSO" })
          : "";
        localStorage.getItem("on-prem") === "true"
          ? base_plans[2].mainFeatures.push({ id: 6, value: "On premise" })
          : "";
        localStorage.getItem("log_streaming") === "true"
          ? base_plans[2].mainFeatures.push({ id: 7, value: "Log streaming" })
          : "";
        localStorage.getItem("log_retention") === "true"
          ? base_plans[2].mainFeatures.push({
              id: 8,
              value: "Custom log retention",
            })
          : "";
      }
      return base_plans;
    },
  },
};
</script>