...
<template>
	<div class="flex h-screen">
		<TransitionRoot as="template" :show="sidebarOpen">
			...
			>
				...
				<div class="fixed inset-0 z-40 flex">
					...
					>
						<DialogPanel
							class="relative flex w-full max-w-xs flex-1 flex-col bg-[#24152C] focus:outline-none"
						>
							<TransitionChild
								as="template"
		...
		<div class="flex min-w-0 flex-1 flex-col overflow-hidden">
			<div class="lg:hidden">
				<div
					class="flex items-center justify-between border-b border-gray-200 bg-base-300 px-4 py-1.5"
				>
					<div>
						<img
							class="h-8 w-auto"
			...
			<div class="relative z-0 flex flex-1 overflow-hidden">
				<main class="flex-1 overflow-y-auto focus:outline-none bg-base-100">
					<!-- Start main area-->
					<div class="py-6 px-4 sm:px-8 lg:px-12">
						<div class="h-full rounded-lg">
							<div class="my-12 max-w-3xl">
								<div
									class="relative rounded-md border border-base-content/20 px-3 py-2 shadow-sm focus-within:border-indigo-600 focus-within:ring-1 focus-within:ring-indigo-600"
								>
									<label
										for="name"
										class="absolute -top-2 left-2 -mt-px inline-block bg-base-200 px-1 text-xs font-medium text-base-content/50"
										>Message text</label
									>
									<input
										type="text"
										...
										v-model="message"
										id="name"
										class="block w-full border-0 p-0 bg-transparent text-base-content placeholder-gray-500 focus:ring-0 sm:text-sm"
							...
							<div class="">
								<div class="mb-6 flex">
									<h1 class="text-xl font-bold text-base-content mr-2">
										Solutions
									</h1>
									<button
										@click="show = !show"
										class="btn-secondary rounded-full px-2 text-sm py-1"
									>
										<NoCodeIcon v-if="show" class="h-4 w-4" />
										<CodeIcon v-else class="h-4 w-4" />
									</button>
								</div>
				...
				<aside
					v-if="show"
					class="relative hidden w-[600px] flex-shrink-1 overflow-y-auto border-l border-base-300 bg-base-200 xl:flex xl:flex-col"
				>
					<!-- Start secondary column (hidden on smaller screens) -->
					<div
						class="inset-0 py-6 mb-6 px-4 sm:px-6 lg:px-8 overflow-y-auto max-h-screen"
					>
